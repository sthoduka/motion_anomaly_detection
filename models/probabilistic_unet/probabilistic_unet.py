'''
Copyright Stefan Knegt (https://github.com/stefanknegt/Probabilistic-Unet-Pytorch)
LICENSE: Apache


2021/03:
modified by Santosh Thoduka to use the network for predicting 2-channel optical flow images
...
class ProbabilisticUnet(nn.Module):
    ...
    def forward(self, flow, flow_future, training=True):
        """
        Construct prior latent space for patch and run patch through UNet,
        in case training is True also construct posterior latent space
        """
        if training:
            self.posterior_latent_space = self.posterior.forward(flow, flow_future)
        self.prior_latent_space = self.prior.forward(flow)
        self.unet_features = self.unet.forward(flow,False)
    ...
    def elbo(self, flow_future, analytic_kl=True, reconstruct_posterior_mean=False):
        """
        ...
        reconstruction_loss = criterion(input=self.reconstruction, target=flow_future)
